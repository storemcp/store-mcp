...
final class Permissions {
	...
	public static function require_cap( AuthContext $context, string $capability, ...$args ): void {
		if ( ! $context->user_can( $capability, ...$args ) ) {
			throw new Tool_Exception(
				esc_html( sprintf( /* translators: %s: capability */ __( 'Missing capability: %s', 'store-mcp' ), $capability ) ),
	...
	public static function require_any( AuthContext $context, array $capabilities ): void {
		...
		throw new Tool_Exception(
			esc_html( sprintf( /* translators: %s: capability list */ __( 'Missing one of capabilities: %s', 'store-mcp' ), implode( ', ', $capabilities ) ) ),
			Server::ERR_FORBIDDEN
		);
	}
	...
	public static function require_woocommerce(): void {
		if ( ! class_exists( 'WooCommerce' ) ) {
			throw new Tool_Exception( esc_html__( 'WooCommerce is not active', 'store-mcp' ), Server::ERR_FORBIDDEN );
		}
	}
}
